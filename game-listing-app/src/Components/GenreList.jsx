...
const GenreList = ({setGenreId, selectedGenre}) => {
  ...
  return (
    <div>
        ...
        {genreList.map((item,index)=>(
            ...
            <div 
            onClick={()=>{setActiveIndex(index); setGenreId(item.id); selectedGenre(item.name)}}
            className={`flex gap-2 items-center mb-2 cursor-pointer hover:bg-gray-300 p-2 group rounded-lg hover:dark:bg-gray-600 ${activeIndex==index?"bg-gray-300 dark:bg-gray-600":null}`}>
                <img src={item.image_background} className={`w-[40px] h-[40px] object-cover rounded-lg group-hover:scale-105 transition-all ease-out duration-300 ${activeIndex==index?"scale-105":null}`} alt="" />
